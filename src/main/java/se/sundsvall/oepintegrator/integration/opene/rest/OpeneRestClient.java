...
public interface OpeneRestClient extends OpeneClient {
	...
	@GetMapping(path = "/api/unsubmittedinstanceapi/getinstances/owner/citizenidentifier/{legalId}", produces = TEXT_XML_CHARSET_ISO_8859_1)
	Optional<byte[]> getUnsubmittedCaseListByCitizenIdentifier(
		@PathVariable final String legalId,
		@RequestParam(name = "showStatus", required = false) final Boolean showStatus);

	@GetMapping(path = "/api/unsubmittedinstanceapi/getinstances/owner/userid/{userId}", produces = TEXT_XML_CHARSET_ISO_8859_1)
